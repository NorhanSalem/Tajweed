...
type ActiveTeacher_TP = {
  title: string;
};

function ActiveTeacher({ title }: ActiveTeacher_TP) {
  ...
  const cols = useMemo<ColumnDef<ActiveTeacher>[]>(
    () => [
      {
        header: "#",
      ...
      {
        header: `${t("Total delay")}`,
        cell: (info: any) => info.renderValue(),
        accessorKey: "delay_time",
      },

      // {
      //   header: `${t("total profit")}`,
      //   cell: (info) => (
      //     <div>{info.row?.original?.teacher_profile?.total_profit}</div>
      //   ),
