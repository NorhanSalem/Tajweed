...
type Specializations_TP = {
  title: string;
};

function Specializations({ title }: Specializations_TP) {
  ...
  const cols = useMemo<ColumnDef<Specializations>[]>(
    () => [
      {
        header: "#",
        ...
        cell: (info) => (
          <div className="flex justify-center">
            {info.row.original.active == 1 ? (
              <p className="bg-[#50cd89] cursor-pointer text-white w-max py-[0.150rem] px-2 rounded-[.325rem] text-[12px]" onClick={()=>handleActive(info.row.original?.id)}>
                {t("active")}
              </p>
            ) : (
              <p className="bg-[#f1416c] cursor-pointer text-white w-max py-[0.150rem] px-2 rounded-[.325rem] text-[12px]"  onClick={()=>handleActive(info.row.original?.id)}>
                {t("notactive")}
              </p>
            )}
          </div>
  ...
  const endpoint = `dashboard/teachers/specializations?${searchParams.toString()}`;
  ...
  const { mutate: deleteSponsor } = useMutate({
    mutationKey: [`dashboard/teachers/specializations/${specializationID}`],
    endpoint: `dashboard/teachers/specializations/${specializationID}`,
    onSuccess: (data: any) => {
      notify("success");
      refetch();
    },
  ...
  const { mutate: activate } = useMutate({
    mutationKey: [`dashboard/teachers/specializations/${specializationID}/activate`],
    endpoint: `dashboard/teachers/specializations/${specializationID}/activate`,
    onSuccess: (data: any) => {
      notify("success");
      refetch();
    },
        ...
        <div className="grid grid-cols-12">
          <div className="col-span-12 ">
            <div className=" flex justify-end items-">
              <div className="">
                <AddButton
                  ...
                  addLabel={`${t("Add Specializations")}`}
                />
              </div>
            </div>
            <Table
            ...
            <div className="flex justify-end mt-3">
              <Paginate
                pagesCount={Specializations?.data?.paginate.total_pages}
